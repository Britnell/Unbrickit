...
class LowPassFilter {
  #y: number | null = null; // raw last input
  #s: number | null = null; // filtered value
...
export class OneEuroFilter {
  ...
  filter(value: number, nowMs: number) {
    if (this.#lastT !== null && nowMs > this.#lastT) {
      ...
      this.#lastT = nowMs;
      const dx = (value - (this.#xfilt.lastValue ?? value)) / dtS;
...
export class LandmarkOneEuro {
  ...
  constructor(
    opts: { minCutoff?: number; beta?: number; dCutoff?: number } = {},
  ) {
    this.opts = opts;
  }
